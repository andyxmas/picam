#! /usr/bin/python
# Originally Written by Peter Nichols to copy files to Google Drive. http://www.itdiscovery.info
...
parser.add_argument('-u', '--username', 
                    help='-u --username. Google Drive Username to Login as. Required!', required=True)
parser.add_argument('-p', '--password', 
                    help='-p --password. Google Drive Password to Login as. Required!', required=True)
parser.add_argument('-l', '--location', default='picam', 
                    help='-l --location. [picame] Google Drive Directory to store the jpg files.', required=False)
parser.add_argument('-f', '--filename', 
                    help='-f --filename The file name/directory for the image file.', required=True)
parser.add_argument('-s', '--single', default=0, type=int, 
                    help='-s --single [1] A single file [0] or all files [1] of that type in the directory', required=False)
parser.add_argument('-m', '--mimetype', default='jpg', choices=['kml', 'jpg', 'txt', 'csv', 'mpg', 'mp4'], 
                    help='-m --mimetype [jpg] The file type of the files: kml, jpg, txt, csv, mpg, mp4', required=False)
...
# commenting out the below, as is looking for all files in directory.
# for now at least, i'm just looking to do this for a specific file
'''
if single == 1:
